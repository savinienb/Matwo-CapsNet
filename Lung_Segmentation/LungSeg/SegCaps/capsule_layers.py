...
class ConvCapsuleLayer(layers.Layer):
    def __init__(self, kernel_size, num_capsule, num_atoms, strides=1, padding='same', routings=3,
                 kernel_initializer='he_normal',is_training=True, **kwargs):
    ...
    def build(self, input_shape):
        ...
        W_shape=[self.kernel_size, self.kernel_size,self.input_num_atoms, self.num_capsule * self.num_atoms]
        self.W = tf.layers.conv2d(ones_kernel,self.kernel_size*self.kernel_size*self.input_num_atoms* self.num_capsule * self.num_atoms, 1, 1,kernel_initializer=self.kernel_initializer,trainable=self.is_training,use_bias=False,reuse=tf.AUTO_REUSE,name='W'+self.namey)
        self.W=tf.reshape(self.W,W_shape)


        b_shape=[1, 1, self.num_capsule, self.num_atoms]
        ones_kernel=tf.ones([1,1,1,1])
        self.b = tf.layers.conv2d(ones_kernel, self.num_atoms*self.num_capsule, 1, 1,kernel_initializer=initializers.constant(0.1),trainable=self.is_training,use_bias=False,reuse=tf.AUTO_REUSE,name='b'+self.namey)
    ...
    def call(self, input_tensor, training=None):
        ...
        biases_replicated = tf.tile(self.b, [conv_height.value, conv_width.value, 1, 1])
...
class DeconvCapsuleLayer(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if self.upsamp_type == 'subpix':
            ...
            self.W = self.add_weight(shape=[self.kernel_size, self.kernel_size,
                                     ...
                                     name='W')
            '''
            W_shape=[self.kernel_size, self.kernel_size,self.input_num_atoms,self.num_capsule * self.num_atoms * self.scaling * self.scaling]
            self.W = tf.layers.conv2d(ones_kernel,self.kernel_size* self.kernel_size*self.input_num_atoms*self.num_capsule * self.num_atoms * self.scaling * self.scaling, 1, 1,kernel_initializer=self.kernel_initializer,trainable=self.is_training,use_bias=False,reuse=tf.AUTO_REUSE,name='W'+self.namey)
            self.W=tf.reshape(self.W,W_shape)
            ...
            self.W = self.add_weight(shape=[self.kernel_size, self.kernel_size,
                                     ...
                                     initializer=self.kernel_initializer, name='W')
            '''
            W_shape=[self.kernel_size, self.kernel_size, self.num_capsule * self.num_atoms, self.input_num_atoms]
            self.W = tf.layers.conv2d(ones_kernel,self.kernel_size* self.kernel_size*self.num_capsule * self.num_atoms* self.input_num_atoms , 1, 1,kernel_initializer=self.kernel_initializer,trainable=self.is_training,use_bias=False,reuse=tf.AUTO_REUSE,name='W'+self.namey)
            self.W=tf.reshape(self.W,W_shape)
        else:
    ...
    def call(self, input_tensor, training=None):
        ...
        if self.upsamp_type == 'resize':
            ...
            out_height = deconv_length(self.input_height, self.scaling, self.kernel_size, self.padding,self.output_padding)
            out_width = deconv_length(self.input_width, self.scaling, self.kernel_size, self.padding,self.output_padding)
        ...
        logit_shape = K.stack([
            input_shape[1], input_shape[0], votes_shape[1], votes_shape[2], self.num_capsule])
        biases_replicated = tf.tile(self.b, [votes_shape[1], votes_shape[2], 1, 1])
    ...
    def compute_output_shape(self, input_shape):
        ...
        output_shape[1] = deconv_length(output_shape[1], self.scaling, self.kernel_size, self.padding,self.output_padding)
        output_shape[2] = deconv_length(output_shape[2], self.scaling, self.kernel_size, self.padding,self.output_padding)
